...
serve as a more complex example of its usage. The project directory structure:
  ...
  - **intree.cmake** - Creates a cmake library target `phiblock-crypto-intree`
    and generates a header at `<BUILD_LOCATION>/include/phiblock/crypto.h`.
  - **CMakeLists.txt** - Declares a new project for creating/installing a
    shared or static `phiblock-crypto` library.

> Downstream projects cannot currently use this project _without_ also using
> the `phiblock-project/phiblock/src/crypto/crypto.h` functions - specifically
...
add_library(all $<OBJECT_TARGETS:phiblock-crypto-intree> all.cpp)
add_library(referenced referenced.cpp)
target_link_libraries(referenced phiblock-crypto-intree)
```
...
A header file is written to `<BUILD_LOCATION>/include/phiblock/crypto.h` which
