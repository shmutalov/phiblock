# Anonymity Networks with PhiBlock

Currently only Tor and I2P have been integrated into PhiBlock. The usage of
...
is made or until `phiblockd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `phiblock-wallet-cli` and
`phiblock-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `phiblockd` the ip/port of a socks proxy provided by a
...
which tells `phiblockd` that ".onion" p2p addresses can be forwarded to a socks
...
```
--add-exclusive-node u7bn5buyzcm4lmbp23urvjijadmplr54xqh75hnrwdd3vmne6mjmzcyd.onion:18010
--add-peer u7bn5buyzcm4lmbp23urvjijadmplr54xqh75hnrwdd3vmne6mjmzcyd.onion:18010
```
...
`--anonymous-inbound`. This option tells `phiblockd` the inbound address, network
type, and max connections:

```
--anonymous-inbound u7bn5buyzcm4lmbp23urvjijadmplr54xqh75hnrwdd3vmne6mjmzcyd.onion:18010,127.0.0.1:28083,25
```

which tells `phiblockd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `phiblockd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `phiblockd` localhost port 30000.
...
`phiblockd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/phiblock
...
--daemon-address u7bn5buyzcm4lmbp23urvjijadmplr54xqh75hnrwdd3vmne6mjmzcyd.onion
```
...
```
HiddenServiceDir /var/lib/tor/data/phiblock
HiddenServicePort 18083 127.0.0.1:28083
```

This will store key information in `/var/lib/tor/data/phiblock` and will forward
"Tor port" 18083 to port 18083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/phiblock/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the PhiBlock network should have
similar timestamps.
...
### Intermittent PhiBlock Syncing

If a user only runs `phiblockd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `phiblockd` as often as possible to conceal when transactions are being sent.
...
`phiblockd` currently selects two outgoing connections every 5 minutes for
...
`phiblockd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `phiblockd` should likely disconnect
