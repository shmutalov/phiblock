...
gsigs = 'https://github.com/phiblock-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('phiblock'):
        subprocess.check_call(['git', 'clone', args.url, 'phiblock'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/phiblock/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'phiblock='+args.commit, '--url', 'phiblock='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/phiblock-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/phiblock/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/phiblock/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/phiblock-project/phiblock', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/phiblock', exist_ok=True)
    os.chdir('builder/inputs/phiblock')
    if args.pull:
